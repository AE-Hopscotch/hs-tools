...
$("body").on("click", "a.read-more.toggle-open", function(e) {
  e.preventDefault()
  $target = $(e.target)
  $($target.attr("href")).animate({
    height: "toggle",
    visible: true
  }, 400)
  $target.removeClass("toggle-open")
  $target.addClass("toggle-closed")
})

$("body").on("click", "a.read-more.toggle-closed", function(e) {
  e.preventDefault()
  $target = $(e.target)
  $($target.attr("href")).animate({
    height: "toggle",
  ...
  $target.removeClass("toggle-closed")
...
const hoverIn = function(person, $headshot) {
  const $caption = $(`.caption.${person}`);
  if ($headshot.hasClass("active")) {
    removeCaption($caption);
    return $(".captions").animate(
      {height: "0px"}
    , 300);
  } else {
    $(".captions").addClass("visible");
    $headshot.addClass("active");
    $caption.show();
    return $(".captions").animate(
      {height: "200px"}
    , 300);
...
const hoverOut = function($caption, person) {
  if ($caption.is(":visible") && !$caption.hasClass(person)) {
    return removeCaption($caption);
  }
};

var removeCaption = function($caption) {
  const captionPerson = $caption.data("person");
  $(`.headshot[data-person='${captionPerson}']`).removeClass("active");
  $caption.removeClass("active");
  return $caption.hide();
};

$(".headshot").on("click", function(e) {
  const headshot = $(e.target).closest(".headshot");
  const person = headshot.data("person");
  hoverIn(person, headshot);
  return Array.from($(".caption")).map((caption) => hoverOut($(caption), person));
});

$(".educators").on("ajax:complete", "form", function(data, status, xhr) {
  $(".educators h3.success").show();
  $(".educators input[type=email]").val("");
  $(".educators input[type=text]").val("");
  return $('input[type=email]').blur();
});

$("#signup form").on("ajax:complete", function(data, status, xhr) {
  $("#signup .success").show();
...
//
// from http://stackoverflow.com/questions/12302300/track-all-outbound-links-in-google-analytics
...
var shareButtonMobile = document.getElementById('share-button-mobile');
var shareCancel = document.getElementById('share-cancel');
var shareFloat = document.getElementById('share-float');
...
//(function() {
    ...
    function loadPIXI(version, index) {
      ...
      var webplayerData = index[version];
    ...
    //window.onload = function() { AE_MOD
        ...
        var iCheck = setInterval(function(){
          if(INDEX && EDITOR_INDEX) {
            ...
            loadPIXI(HS_WEBPLAYER_VERSION, Object.assign(INDEX, EDITOR_INDEX));
          }
        }, 1000/30);
    //};
 // })(); AE_MOD [36157]
