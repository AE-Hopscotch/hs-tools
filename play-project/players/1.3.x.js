            ...
            switch (t.type) {
			  ...
			  case HSBlockType.None: //AE_MOD
				if (/^_ae_webplayer_action:/g.test((c[0]||"").value)){
					AE_MOD.webplayer_action(c[0].value.split('_ae_webplayer_action:')[1], ((c[1])?c[1].computedValue(this):undefined));
				}
				break;
            }
            ...
            switch (e.type) {
              ...
              case o.HSBlockType.PlaySound:
				function notePath (val) {
					...
					var i = (/^(low-|high)?[a-zA-Z](sharp|flat)?$/.test(val)) ? ({"-1": "", "0": "new/", "1": "guitar/"})[(u[2])?u[2].computedStringValue(t):'0'] : ((isCustom)?"custom/":"");
