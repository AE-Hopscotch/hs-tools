    ...
    var eventNamespace = "." + pluginName,
        ...
        internal = {
            ...
            emulateTabbing: function($from, offset) {
				...
				if (!$next[0].classList.contains('tab-go-back')){
					$next.focus();
					
					//Deal with Scrolling Issues
					if (offset < 0){
						$next = internal.findNextFocusable($from, 2*offset);
						$next.focus();
						$next = internal.findNextFocusable($from, offset);
						$next.focus();
					}
					
					if ($next[0].classList.contains('tab-skip')){
						$next = internal.findNextFocusable($from, 2 * offset);
						$next.focus();
					}
				}
