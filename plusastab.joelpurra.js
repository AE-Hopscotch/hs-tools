...
(function($, namespace)
{
    ...
    function checkEmulatedTabKeyDown(event) {
		...
		var backwards = !options.key.indexOf(event.keyCode);
		
		if (backwards && document.activeElement.className.match("no-reverse") == "no-reverse"){
			return;
		}
		
        wasDone = performEmulatedTabbing(true, backwards, $target);
