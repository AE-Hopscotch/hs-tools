...
const letterCasing = {
	...
	pascalToTitle: function(string) {
		...
		return string.replace(/(^\w)|(?<=[a-z])([A-Z])/g,function(m0,m1,m2){return m1?m1.toUpperCase():" "+m2;});
	}
}
...
if (editor.useBlockRender) {
		...
		editsave: function(forceOpen) {
			...
			try {
				...
				if (JSON.parse(savetext).type == 123) {
					...
					(projectDict.abilities[(data.controlScript||{}).abilityID]||{}).name = data.description;
				}
		...
		play: function() {
			if (editor.project.canAddPlayer) {
				...
				worker.onmessage = function(msg) {
					...
					playFrame.style = "display:absolute;position:fixed;width:100%;height:100%;left:0;bottom:-105%;transition:bottom 0.5s;z-index:100;";
	...
	function replaceRender(data, tct, desc) {
		...
		document.getElementById("blocks-container-resizer").innerHTML = '<div class="drag-handle"><span label="blockrender">Block Rendering: ' + desc + (desc!="Entire Project"&&tct?' <a title="See Entire Project" href="javascript:replaceRender(-1)">&ic; <i class="fa fa-list-alt"></i></a>':'')+' <a class="refresh-btn" title="Refresh Project" href="javascript:editor.blockrender.refresh()">&ic; <i class="fa fa-refresh"></i></a></span><span hidden label="renderjson">JSON Project Render</span></a><a title="Toggle Code Viewing" style="position:absolute;top:3px;right:30px;" href="javascript:editor.display.renderjson()"><i class="fa fa-code"></i></a><a title="Run the Project" style="position:absolute;top:3.5px;right:6px;" href="javascript:editor.project.play()"><i class="fa fa-play"></i></a></div><div id="blocks-container" class="bl-container" style="padding: 36px 8px;" data-group="'+tct+'"></div><div hidden id="renderjson-container"></div><div class="resizer"></div>';
...
if (editor.useFileSysCode) {
	editor.traits = {
		...
		updateFields: function() {
			...
			function getSbAbility(){
				const hasSbAbilityName = !hsProject.abiltiies ? null : (hsProject.abilities.repeatEach(a=>{if(a.name && checkAbility.secretBlocks(a).contains) return (a.name.length > 29) ? a.name.substr(0,27)+"...": a.name})||[]).removeNull()[0];
				const fullSbAbilityName = !hsProject.abiltiies ? null : (hsProject.abilities.repeatEach(a=>{if(a.name && checkAbility.secretBlocks(a).newest) return (a.name.length > 29) ? a.name.substr(0,27)+"...": a.name})||[]).removeNull()[0];
				editor.project.secretBlocks = fullSbAbilityName ? "new" : hasSbAbilityName ? "old" : "none";
				return fullSbAbilityName ? fullSbAbilityName + ' <i title="Up to date" class="fa fa-fw fa-check"></i>' : hasSbAbilityName ? hasSbAbilityName + ' <i title="Out of date" class="fa fa-fw fa-refresh"></i>' : '<i>Not added</i>';
	...
	editor.fileReader = {
		getContents: function(method, input, fn, properties) {
			...
			document.getElementById(input.id + "-label").innerHTML = filename.replace(new RegExp("(?<=^.{"+(properties.maxDisplayLength||10**16)+"}).*"),"\u2026")||"No file chosen";
			var ext;
			try {
	...
	function importLink() {
		...
		XHR.fetch("https://c.gethopscotch.com/api/v1/projects/" + input, function(r,s){
			if (s != 200) {
	...
	document.getElementById("pAct-preset-file-btn").addEventListener("click", function(event){
		...
		const changeFn = async function(){
			...
			function contentHandler(content, filename, ext) {
				...
				document.getElementById("pAct-preset-file-label").innerText = presetArray.length==0?"No file chosen":presetArray.length==1?filename.replace(new RegExp("(?<=^.{"+(20)+"}).*"),"\u2026")+ext:presetArray.length+" files chosen";
				return;
			}
