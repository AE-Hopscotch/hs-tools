				...
				setTimeout(function(){
					...
					worker.onmessage = function(msg) {
						if (msg.data.status === "in progress") {
							...
							p = msg.data.hsProject, projectDict = msg.data.projectDict;
							replaceRender(-1);
							updateDrawers();
							popup.close();
							goButton.removeAttribute("disabled");
...
if (editor.logConsoleMesg) {
	...
	console.log('%cHopscotch Project Builder, beta 1.2.2 r1 %c \u2063 Made by Awesome_E ¯\\_(ツ)_/¯','display:block; padding: 4px 6px; border: 4px solid red; background-color: salmon; color: white; font-weight: bold;','');
}