...
const blockLabels = {
  ...
  7000: ['event', 'game starts'], // Event operator start
  ...
  8000: ['<ps><span><i class="fa fa-fw fa-cubes"></i> Object</span></ps>'],
...
let oldProjAlerted = false
function jsonToHtml (block, isNested, keepClosed) {
  isNested = isNested || false
  keepClosed = keepClosed || false

  let sortGroup = 'blocks'
  let blockParent = activeEditBlock
  let myBlockType
  if (blockParent && blockParent.parentNode && blockParent.parentNode.parentNode && document.querySelector('.edit-box').style.display === 'block') {
    myBlockType = (blockParent.parentNode.id === 'blocks-container') ? document.getElementById('blocks-container').getAttribute('data-group') : blockParent.parentNode.parentNode.getAttribute('data-group')
  }
  if (block.scripts) {
  ...
  const callerFn = (arguments.callee || {}).caller || {}
  ...
  let myScripts
  if (activeEditBlock) {
    const myData = JSON.parse(activeEditBlock.getAttribute('data'))
    if (myData) myScripts = (myData.controlScript) ? new RegExp(('^(' + myData.controlScript.abilityID + '|' + (myData.controlFalseScript || { abilityID: '' }).abilityID).replace(/\|$/, '') + ')$', 'm') : /$.^/
  }
  while (blockParent && !blockParent.classList.value.match(/\b(crule|obj)\b/) && blockParent !== document.getElementById('blocks-container-resizer')) {
    // Check if a block is nested
    const isFalse = isFalseScript(blockParent.parentNode)
    blockParent = (blockParent.parentNode || { parentNode: null }).parentNode || document.getElementById('blocks-container-resizer')
    if (!blockParent.getAttribute('data')) {
      blockParent = document.getElementById('blocks-container-resizer')
      console.log('no data detected, defaulting to scenes outer parent')
    }
    if (blockParent && !blockParent.classList.value.match(/\b(crule|obj)\b/) && blockParent !== document.getElementById('blocks-container-resizer') && myScripts.test((JSON.parse(blockParent.getAttribute('data'))[(isFalse) ? 'controlFalseScript' : 'controlScript'] || { abilityID: JSON.parse(blockParent.getAttribute('data')).abilityID }).abilityID)) isNested = true
  }
  ...
  for (let i = 0; i < (block.parameters || []).length; i++) {
    ...
    function doParameter (d) {
      function hsvToRgb (str) {
        ...
        const h = Math.max((Number(arr[0])) / 360, 0); const s = Math.max((Number(arr[1]) % 101) / 100, 0); const v = Math.max((Number(arr[2]) % 101) / 100, 0)
        let r, g, b
      ...
      function getTrait (id) {
        const name = (blockLabels[(projectDict.traits[id] || {}).HSTraitTypeKey] || [])[0]
        return (name || '').htmlEscape() || '<span style="color:red;">unknown</span>'
      }
      ...
      if (!d.datum) {
        ...
        if (d.type === 44 && /HSB\(\s?(-?[0-9]*?\.?[0-9]*?,?\s?){3}\)$/.test(d.value) && !d.variable) {
          ...
          switch (d.type) {
            case 50: {
              const ep = projectDict.eventParameters[d.variable] || {}
              let innerText
              ep.type = ep.type || ep.blockType
              if (ep.type === 8000) {
                const o = projectDict.objects[ep.objectID]
                if (o && o.type) {
                  innerText = `<ps>${(o.type === 1 ? '<img width="36" src="../images/character_sprite_strip.png" style="object-position:0 -30px"/>' : doParameter({ datum: { type: o.type } }).match(/<i class="fa fa-photo".*?<\/i>|<img style="object-position.*?\/>/)[0])}<span>${o.name}</span></ps>`
                } else {
                  console.log('no type', o)
                }
              } else {
                innerText = `<ps><span>${ep.description || blockLabels[(projectDict.eventParameters[ep.id] || {}).type]}</span></ps>`
              }
              return innerText
            }
            default: {
              if (d.variable) {
                let vClass = 'val'
                let o = projectDict.objects[(projectDict.variables[d.variable] || {}).objectIdString] || projectDict.objects[(projectDict.variables[d.variable] || {}).HSObjectIDString]
                let objectLabel = ''
      ...
      if (d.datum.HSTraitTypeKey >= 2e3 && d.datum.HSTraitTypeKey < 4e3) {
        let objectLabel = (blockLabels[d.datum.HSTraitObjectParameterTypeKey] || '')
        if (d.datum.HSTraitObjectParameterTypeKey === 8000) {
          const o = projectDict.objects[d.datum.HSTraitObjectIDKey] || { name: 'Object' }
      ...
      if (!/operator/i.test(d.datum.block_class)) {
        if (d.datum.type === 1) {
          ...
          const customObjectId = (d.datum && d.datum.customObject) || (block && block.customObjectID)
          ...
          return `<ps><op class="fw cm">${(d.datum.type === 2000) ? image : '<img style="object-position:0 calc(var(--ff-obj-scale) * ' + (d.datum.type < 166 || d.datum.type > 3e3 ? d.datum.type - 2835 * (d.datum.type >= 3e3) : -1) * -30 + 'px)" src="../images/character_sprite_strip.png" width="36"/>'}</op></ps>`
        }
      }
      // Random Color
      ...
      if (d.datum.type < 2e3 || (d.datum.type >= 4e3 && d.datum.type < 6e3) || (d.datum.type >= 7e3 && d.datum.type < 8e3) || (d.datum.type >= 9e3 && d.datum.type < 10e3)) {
        const isRule = (d.datum.type >= 7e3 && d.datum.type < 8e3); const isTextOp = (d.datum.type >= 9e3 && d.datum.type < 10e3)
        ...
        const i = 0; return `<ps><op class="${className} ${isTextOp ? '' : 'cm'}">
  ...
  if (/control/i.test(block.block_class)) {
    ...
    if (block.customRuleID) {
      ...
      if (!keepClosed) {
        if (block.abilityID) {
          const a = projectDict.abilities[block.abilityID]
          if (a) {
            const blockInfo = jsonToHtml(a, true, false)
        ...
        (Object.keys(block.rules || {}) || []).repeatEach(rule => {
          ...
          const blockInfo = jsonToHtml(r, true, true)
          nestedHTML += '<div class="' + blockInfo.classList + '" data="' + blockInfo.data.htmlEscape() + '" data-id="' + blockInfo.id + '" data-group="' + blockInfo.sortGroup + '">' + blockInfo.innerHTML + '</div>'
        })
      } else {
      ...
      if (!keepClosed) {
        (block.objects || []).repeatEach((o) => {
          ...
          const blockInfo = jsonToHtml(o, true, true)
          nestedHTML += '<div class="' + blockInfo.classList + '" data="' + blockInfo.data.htmlEscape() + '" data-id="' + blockInfo.id + '" data-group="' + blockInfo.sortGroup + '">' + blockInfo.innerHTML + '</div>'
        })
      } else {
        elmClass = elmClass.replace('collapsible-container', (nestedUuidList.indexOf(trueScript) !== -1) ? 'disabled' : '')
    ...
    const a = projectDict.abilities[trueScript]
    if (a) {
      ...
      if (!keepClosed && a && nestedUuidList.indexOf(trueScript) === -1) {
        ...
        (Object.keys(a.blocks || {})).repeatEach((k) => {
          const b = a.blocks[k] || {}
          const blockInfo = jsonToHtml(b, true, (b.type === 123))
    ...
    if (!addedToHtml && (trueScript || block.rules || (block.text != null && block.objectID)) && block) {
      const blockInfo = jsonToHtml({}, true, keepClosed)
    ...
    if (block.type === 124) {
      let nestedHTML = '<div class="collapsible">'
      const a = projectDict.abilities[falseScript]
      if (a) {
  ...
  const webId = block.web_id
  if (!block.objects) delete block.web_id
  const cs = block.controlScript; myScripts = null
  if (cs) myScripts = '(' + cs.abilityID + (block.controlFalseScript ? '|' + block.controlFalseScript.abilityID : '') + ')'
  return { id: myScripts || webId || block.id || block.objectID || '', classList: elmClass || '', data: JSON.stringify(block), innerHTML: innerHTML, sortGroup: sortGroup, focusType: myBlockType, scripts: myScripts }
}
