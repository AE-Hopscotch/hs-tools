...
document.body.addEventListener('keydown', function (e) {
  ...
  const noteHotkey = 'awsedftgyhujkp[]\\1234567890-='.indexOf(eventKey.toLowerCase())
  switch (eventKey) {
    case '<':
      if (!currentPianoElm) return
      if (currentPianoElm.previousElementSibling && !currentPianoElm.previousElementSibling.hidden) {
        currentPianoElm.previousElementSibling.classList.add('selected')
        currentPianoElm.classList.remove('selected')
      }
      break
    case '>':
      if (!currentPianoElm) return
      if (currentPianoElm.nextElementSibling && !currentPianoElm.nextElementSibling.hidden) {
        currentPianoElm.nextElementSibling.classList.add('selected')
        currentPianoElm.classList.remove('selected')
      }
      break
  }
...
document.body.addEventListener('keyup', function (e) {
  ...
  const lastRhythmBtn = document.querySelector('.wait-btns button.selected'); let targetElm
  ...
  if (targetElm.classList.contains('active')) {
    ...
    const ev = new MouseEvent('click', { shiftKey: e.shiftKey !== e.getModifierState('CapsLock') })
    ev.initEvent('click')
...
document.body.addEventListener('keydown', function (e) {
  ...
  const textarea = document.getElementById('music-inputs').querySelector('textarea')
...
function createTimeMap (str) {
  let index = 0; let cumulativeTime = 0; const noteMap = {}; let safetyExecCap = 0; let lastInstrument = '@p'
  while (index < str.length) {
    ...
    if (char.match(/[-=+>]/)) {
      ...
      const waitString = str.match(new RegExp(`(?:.|\\n){${index}}(\\d+)`))[1]
      ...
      const commentMatch = str.match(new RegExp(`(?:.|\\n){${index}}((?:.|\\n)*?(?:\\*\\/|$))`))
...
function windowMergeTracks (e) {
  const combinedMap = {}; const errors = []; const textareaList = document.querySelectorAll('.textbox-float.selected[data-float-id] textarea')
...
function setPlayerTrack (id) {
  const musicString = document.querySelector(`.textbox-float[data-float-id="${id}"] textarea`).value
  localStorage.setItem('pData__AE_MUSIC_READER', musicString)
  const frameParent = document.getElementById('iframe-float')
...
document.getElementById('tempo-input').addEventListener('input', function (e) {
  ...
  if (Number(e.target.innerText) > 600) {
    e.target.innerText = '600'
    moveCursorToEnd()
  } else if (Number(e.target.innerText) < 0) {
...
function windowSelectOne (id, bool) {
  ...
  const checkedRatio = document.querySelectorAll('.textbox-float table tbody input:checked').length / document.querySelectorAll('.textbox-float table tbody input').length
  const totalInput = document.querySelector('.textbox-float table thead input')
...
function windowDelayedClick (id, playAnimation) {
  if (playAnimation == null) playAnimation = true
...
function reorganizefloatingZ (event) {
  const draggables = document.querySelectorAll('.textbox-float')
...
function loadFile (file, position) {
  ...
  reader.addEventListener('load', function () {
    ...
    midiTracks.forEach((t, i) => {
      ...
      const instrument = t.instrument.family + ' ' + t.instrument.name
      const computedInstrument = midiInstrumentDict[instrument.match(/^\w+/)[0]] || midiInstrumentDict[instrument.match(/^\w+\s\w+/)[0]] || 'NONE'
      ...
      const lowestUnderAmt = 42 - t.notes.reduce((a, b) => Math.min(a, b.midi), Infinity)
      const highestOverAmt = t.notes.reduce((a, b) => Math.max(a, b.midi), 0) - 84
      let noteOffset = 0
      ...
      const trackName = formatTitle(file.name.replace(/\.\w+$/, '')) +
...
function loadMidi (files) {
  const file = files[0]; const inputPos = document.getElementById('midi-import-file-input').style
